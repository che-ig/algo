...
class Solution_2:
    def sumSubarrayMins(self, arr: list[int]) -> int:
        ...
        # left[i] хранит индекс ближайшего элемента слева, который строго меньше arr[i]
        left = [-1] * n
        stack = []
        for i in range(n):
            # Пока стек не пуст и элемент на вершине стека >= текущего, удаляем его
            while stack and arr[stack[-1]] >= arr[i]:
                stack.pop()
            # Если стек не пуст, то верхний элемент — это ближайший меньший слева
            if stack:
                left[i] = stack[-1]
            # Добавляем текущий индекс в стек
            stack.append(i)

        # right[i] хранит индекс ближайшего элемента справа, который меньше или равен arr[i]
        right = [n] * n
        stack = []
        for i in range(n - 1, -1, -1):
            # Пока стек не пуст и элемент на вершине стека > текущего, удаляем его
            while stack and arr[stack[-1]] > arr[i]:
                stack.pop()
            # Если стек не пуст, то верхний элемент — это ближайший меньший или равный справа
            if stack:
                right[i] = stack[-1]
            # Добавляем текущий индекс в стек
            stack.append(i)
        ...
        result = 0
        for i in range(n):
            # Количество подмассивов, где arr[i] является минимумом
            count = (i - left[i]) * (right[i] - i)
            # Прибавляем вклад элемента к общему результату с учетом модуля
